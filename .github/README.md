# CI/CD setup — reservation-service
...
| `SERVICE_URL_DEV` | optional | — | Public URL of the dev Cloud Run revision (e.g. `https://reservation-service-dev-abc.a.run.app`). When set, the smoke job probes `/healthz`. |
...
REPO_NAME=reservation-service        # Repository name
SA_NAME=reservation-deployer
...
```bash
gcloud run deploy reservation-service-dev \
...
gcloud run services describe reservation-service-prod --region=$REGION \
  --format='value(status.traffic)'

# Point all traffic back at it
gcloud run services update-traffic reservation-service-prod \
  --region=$REGION \
